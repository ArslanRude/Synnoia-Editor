export const defaultWebPages = [
  {
    label: 'YouTube',
    icon: '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15.84 3.45c-.15-.6-.65-1.05-1.25-1.2C13.05 2 8 2 8 2s-5.05 0-6.6.25c-.6.15-1.1.6-1.25 1.2C0 4.95 0 8 0 8s0 3.05.15 4.55c.15.6.65 1.05 1.25 1.2C2.95 14 8 14 8 14s5.05 0 6.6-.25c.6-.15 1.1-.6 1.25-1.2C16 11.05 16 8 16 8s0-3.05-.16-4.55zM6.4 10.5V5.5l4.8 2.5-4.8 2.5z" fill="#FF0000"/></svg>',
    validate(url: string) {
      const patterns = [
    ...
    transformURL(url: string) {
      ...
      if (url.includes('youtube.com/watch?v=')) {
        ...
        videoId = url.match(/embed\/([a-zA-Z0-9_-]+)/)?.[1] || ''
      }
  ...
  },
  {
    ...
    validate(url: string) {
      const domain = /^https?:\/\/v\.qq\.com\//
      const coverPath = /\/cover\/[a-zA-Z0-9]+\/[a-zA-Z0-9]+/
      return domain.test(url) && coverPath.test(url)
    },
    transformURL(url: string) {

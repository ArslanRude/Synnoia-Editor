    ...
    <template v-for="(group, index) in shortcuts" :key="index">
      ...
      <ul class="keyboard-shortcuts">
        <li v-for="(item, i) in group.items" :key="i">
          <span :class="item.className ? 'keyboard-shortcut-' + item.className : ''">
            <icon v-if="item.icon" :name="item.icon" />
            <span v-else-if="item.html" class="shortcut-icon" v-html="item.html">
            </span>
...
.keyboard-shortcuts {
  li {
    ...
    &:not(:last-child) {
      border-bottom: solid 1px var(--arslan-border-color);
    }
    ...
    &:hover {
      ...
      .keyboard-shortcut kbd {
        border-color: var(--arslan-text-color);
      }
    }
  }
  ...
  blockquote {
    padding-left: 10px;
    border-left: solid 3px var(--arslan-border-color);
  }

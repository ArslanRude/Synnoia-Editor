<template>
  ...
  >
    <template #content>
      <div class="arslan-emojis-container narrow-scrollbar">
        <template v-for="(group, index) in options.dicts?.emojis" :key="index">
          <div class="arslan-emojis-group-title" v-text="l(group.label)"></div>
          <div class="arslan-emojis-group-container">
            <div
              v-for="(item, i) in group.items.split(' ')"
              :key="i"
              class="arslan-emojis-group-item"
...
.arslan-emojis-group {
  ...
  &-title {
    ...
    padding-left: calc(var(--arslan-popup-content-padding) + 5px);
  ...
  &-container {
    display: flex;
    flex-wrap: wrap;
    padding: 10px var(--arslan-popup-content-padding);
    overflow: auto;
    gap: 2px;
  }
  &-item {

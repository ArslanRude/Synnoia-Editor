  ...
  <div v-if="$toolbar.show" class="flex relative mx-3 mb-2 select-none">
    ...
    <div :class="[
      ...
      <t-popup v-if="options.document.readOnly !== true" v-model="statusPopup" :attach="container" trigger="click"
        ...
        <t-button class="p-0 min-w-0 h-auto hover:bg-gray-100 dark:hover:bg-gray-700 [&:not(:last-child)]:mr-[3px]"
          :class="{ 'bg-gray-100 dark:bg-gray-700': statusPopup }" variant="text" size="small">
          <span class="flex items-center text-xs cursor-pointer">
            <span class="w-2.5 h-2.5 rounded-full bg-red-600 mr-1.5" :class="{ 'bg-red-600': !online }"></span>
            <span class="text-gray-500 dark:text-gray-400">
              <span v-if="savedAt" v-text="t('save.savedAtText', { time: timeAgo(savedAt) })"></span>
              <span v-else class="text-red-500" v-text="t('save.unsaved')"></span>
            </span>
          </span>
        </t-button>
        <template #content>
          <div
            ...
            <div class="flex space-x-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
      ...
      <t-dropdown trigger="click" size="small" placement="bottom-right" :popup-props="{
        ...
        <t-button class="rounded transition-colors" variant="text" size="small">
          <div class="flex items-center p-2 rounded-xl hover:bg-primary-light ">
            ...
            <span class="flex items-center sm:inline hidden ml-1 text-text-light dark:text-text-dark">
              {{ t('toolbar.toggle') }}
            </span>
          </div>
        </t-button>

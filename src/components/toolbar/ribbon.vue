<template>
  <div class="w-full">
    <div v-if="menus.length > 1" class="flex px-2.5 py-1 gap-2 z-[1] bg-transparent">
      <MorphingTabs :tabs="menuTabs" :active-tab="currentMenu" @update:active-tab="changeMenu" />
    </div>
    <toolbar-scrollable ref="scrollableRef"
      class="w-full p-2.5 box-border bg-highlight-light dark:bg-highlight-dark rounded-2xl border border-gray-200 dark:border-secondary-dark shadow-sm">
      <div class="flex h-14 shrink-0">
        <template v-if="currentMenu === 'base'">
          <div class="px-5 shrink-0 first:pl-0 empty:hidden">
            <div class="flex items-center [&>*:not(:last-child)]:mr-1.5 mb-1.5">
              <menus-toolbar-base-undo />
              <menus-toolbar-base-redo />
            </div>
            <div class="flex items-center [&>*:not(:last-child)]:mr-1.5">
...
const menuTabs = computed(() => props.menus.map((menu) => menu.value))
